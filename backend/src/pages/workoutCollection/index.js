import { getAllCollection, deletecollection, deleteMultCollection, updateCollectionStatus } from '../../ApiServices';
...
const WorkoutCollection = () => {
  ...
  const list = async () => {
    ...
    await getAllCollection()
      .then((response) => {
        ...
        setbaseurl(`${process.env.REACT_APP_API_KEY_IMAGE_WORKOUT_PATH}`);
      })
      .catch((err) => {
  ...
  const columns = [
    {
      ...
      options: {
        customBodyRender: (image) =>
          image ? (
            <img
              src={`${process.env.REACT_APP_API_KEY_IMAGE_WORKOUT_PATH}${image}`}
              alt={image}
            ...
            <Switch
              ...
              onChange={() => {
                if (userRole == 1) {
                  ...
                  updateCollectionStatus(data, _id)
                    .then(() => {
              ...
              <Icons.Edit
                ...
                onClick={() => {
                  if (userRole == 1) {
                    ...
                    navigate('/workoutCollection/manage', { state: { editdata: editdata, imageurl: baseurl } });
                  } else {
                    toast.error(
  ...
  const options = {
    customToolbarSelect: (selectedRows, data) => (
      <SelectedRowsToolbar selectedRows={selectedRows} data={data} columns={columns} datatableTitle="Workout Collection" />
    )
  };
          ...
          <div className="text-container">
            ...
            <div className="right-text">
              <Button
                ...
                onClick={() => {
                  if (userRole == 1) {
                    navigate('/workoutCollection/manage');
                  } else {
                    toast.error(
                      'Sorry, you do not have permission to access this feature.Please contact your administrator for assistance.'
                    );
